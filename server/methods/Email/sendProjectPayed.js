Meteor.methods({
	sendProjectPayed(projectId, val) {
	...
	'					Сообщаем вам о поступлении оплаты по проекту '+
	project.name + ' - ' + val + 'руб' +
	'					</td>'+
	...
	'	</html>';
		...
		Email.send({
			...
			subject: 'Поступила оплата по проекту ['+ project._id +'] - ' + val + ' руб.' ,
			html: html
		});
		...
		Email.send({
			to: useremail,
			from: 'tecweb@yandex.ru',
			subject: 'Поступила оплата по вашему проекту ['+ project._id +'] - ' + val + ' руб.',
			html: html
		});
