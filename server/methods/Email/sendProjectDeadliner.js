Meteor.methods({
	sendProjectDeadline(taskId) {


		check(taskId, String);
        //
		//
		const currentApplication = Projects.findOne(taskId);
	...
	'				Время завершения проекта изменилось'+
	'				</td>'+
	...
	'					Сообщаем вам, что время до завершения проекта '+
	...
	'	</html>';
		...
		var desinger = Meteor.users.findOne(currentApplication.desinger);
		var desingermail = desinger.emails[0].address;

		var developer = Meteor.users.findOne(currentApplication.developer);
		var developermail = developer.emails[0].address;

		var manager = Meteor.users.findOne(currentApplication.manager);
		var managermail = manager.emails[0].address;

		var user = Meteor.users.findOne(currentApplication.user);
		var userrmail = user.emails[0].address;
		...
		Email.send({
			...
			subject: 'Изменнеие дедлайна проекта ['+ taskId +']',
			html: html
		});
		// отправляем сообщение исполнителю
		Email.send({
