Meteor.methods({
	sendNewPartner(userToken) {

		var user = Meteor.users.findOne({partner: userToken});
		var email = user.emails[0].address;

		var currentUser = Meteor.users.findOne(this.userId);
		if(user.emails[0].address == currentUser.emails[0].address){
			throw new Meteor.Error('Партнёрская ссылка не сработала, код 28', { method: 'sendNewPartner' });
		}

		Meteor.users.update(this.userId, {
			$set: {parent: user._id, balance: 1000}
		});
		...
		var html =
			...
			'					Поздравляем! По вашей партнёрской ссылке зарегистрирован новый клиент. Напоминаем, что вам полагается 10% от его первого оплаченного заказа. Ожидаем вместе с вами этого и сразу же уведомим вас!'+
			'					</td>'+
			'					</tr>' +
			'<tr style="font-family:    Helvetica,Arial,sans-serif; box-sizing: border-box; font-size: 14px; margin: 0;"><td class="content-block" style="font-family:    Helvetica,Arial,sans-serif; box-sizing: border-box; font-size: 14px; vertical-align: top; margin: 0; padding: 0 0 20px;" valign="top">'+
		...
		return Email.send({
			...
			subject: 'Регистрация по партнерской ссылке',
			html: html
		});
	}
});
