Meteor.methods({
	sendTaskDevStatus(id) {
		...
		if(currentApplication.devstatus == 1) {
			var html1 =
				...
				'					Программист считает что задание выполнено, необходимо тчательно это проверить' +
				'					</td>' +
			...
			Email.send({
				...
				subject: 'Задача выполнена и требует проверки (Код) [' + currentApplication.name + ']',
				html: html1
			});
			...
			Email.send({
				to: developermail,
				from: 'tecweb@yandex.ru',
				subject: 'Задача требует доработки (Код) [' + currentApplication.name + ']',
				html: html2
			});
		}
	}
