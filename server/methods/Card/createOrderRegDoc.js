...
Meteor.methods({
	createOrderRegDoc(orderId) {


		const order = OrdersList.findOne(orderId);
		if (!order){
			throw new Meteor.Error('error-invalid-name', 'Проект не существует', { method: 'createOrderRegDoc' });
		}

		if (order.user != this.userId  && Meteor.user().roles != 'admin') {
			throw new Meteor.Error('error-invalid-name', 'Нет доступа', { method: 'createOrderRegDoc' });
		}
		...
		var pdf = {
			info: {
				title: 'Договор на ежемесячное собпровождение сайта №000'+order.number+'б от '+ moment(order._createdAt).format('DD-MM-YYYY'),
				author: 'Stuurgurs',
				subject: 'Theme',
			...
			content: [
				{
					text: 'Договор на ежемесячное собпровождение сайта №000'+order.number+'б от '+ moment(order._createdAt).format('DD-MM-YYYY'),
					fontSize: 16,
					bold: true,
					alignment: 'center',
					margin: [30, 10, 30, 40]
