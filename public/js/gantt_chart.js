...
// require("/js/moment-duration-format.js");
                ...
                for (var j = 0; j < data[i].series.length; j++) {
                    ...
                    if(data[i].series[j].sub_series) {
                        ...
                        $.each(sub_series, function($key,$value) {
                            ...
                            var title = $value.title ? $value.link ? '<a href="'+$value.link+'" title="'+$value.link+'">' + $value.title + '</a>': $value.title : moment.duration(size,'days').humanize();
                            if($value.link) {
                        ...
                        var title = series.title ? series.link ? '<a href="'+series.link+'" title="'+series.link+'">' + series.title + '</a>': series.title : moment.duration(size,'days').humanize();
                        if(series.link) {
                            block.append(jQuery("<div>", { "class": "ganttview-block-text" }).html( title ));
                        } else {
                            block.append(jQuery("<div>", { "class": "ganttview-block-text" }).text( title ));
